...
public class TLeaderStayMoneyManagerImpl  implements TLeaderStayMoneyManager {
		...
		if (phoneMap != null && phoneMap.get("member_id") != null && !"".equals(phoneMap.get("member_id").toString())) {
			...
			if (memberId != null && memberId > 0L && (stayMoneyList == null || stayMoneyList.size() <= 0)) {
				...
				if (waitingAmount != null && waitingAmount > 0L) {
					...
					resultMap.put("message", "写流水成功");
					
				}else{
					resultMap.put("status", 0);
					resultMap.put("message", "返现金额小于0");
				}
			}else{
				resultMap.put("status", 0);
			...
			if (liushuiList != null && liushuiList.size() > 0) {
				...
				if (memberId != null && memberId > 0L) {
					...
					resultMap.put("status", 0);
					resultMap.put("message", "订单MemberId为空");
	 ...
	 */
	@Override
	public void cancelOrderInsertStayByHDF(Map<String, Object> phoneMap, TOrder order) throws SQLException {
		if (phoneMap != null && phoneMap.get("member_id") != null
				&& !"".equals(phoneMap.get("member_id").toString())) {
			// 3:流水不存在该流水的;
			TLeaderStayMoneyExample _TLeaderStayMoneyExample = new TLeaderStayMoneyExample();
			_TLeaderStayMoneyExample.createCriteria().andOrderIdEqualTo(order.getId())
					.andTypeEqualTo(2);
			List<TLeaderStayMoney> liushuiList = tleaderstaymoneydao
					.selectByExample(_TLeaderStayMoneyExample);
			if (liushuiList != null && liushuiList.size() > 0) {
				System.out.println("订单【" + order.getOrderSn() + "】的取消订单待入账流水已经存在了");
			} else {
				// 该订单属于秀粉订单,找领袖写流水；
				Long memberId = Long.valueOf(phoneMap.get("member_id").toString());
				if (memberId != null && memberId > 0L) {
					...
					if (waitingAmount != null && waitingAmount > 0L) {
						...
						_TLeaderStayMoney.setOrderId(order.getId());
						_TLeaderStayMoney.setRelationMemberId(Long.valueOf(phoneMap
								.get("member_id").toString()));// 要返现领袖用户ID
						_TLeaderStayMoney.setStatus(0);// 未入账
						...
						Map<String, Object> tuijianParm = new HashMap<String, Object>();
						tuijianParm.put("memberId", memberId);
						Long tuijianMemberId = (Long) opensqldao.selectForObjectByMap(
								tuijianParm, "t_member.selectTuijianByMemberId");
						if (tuijianMemberId != null && tuijianMemberId > 0L) {
							// 推荐人存在
							Double tuijianAmount = (Double) opensqldao.selectForObjectByMap(
									countMap, "t_order_item.countFanxianAmountAgent");
							if (tuijianAmount > 0) {
								TLeaderStayMoneyExample _TuijianTLeaderStayMoneyExample = new TLeaderStayMoneyExample();
								_TuijianTLeaderStayMoneyExample.createCriteria()
										.andOrderIdEqualTo(order.getId()).andTypeEqualTo(2)
										.andRelationMemberIdEqualTo(tuijianMemberId);
								List<TLeaderStayMoney> tuijianLiushuiList = tleaderstaymoneydao
										.selectByExample(_TuijianTLeaderStayMoneyExample);
								if (tuijianLiushuiList != null && tuijianLiushuiList.size() > 0) {
									System.out.println("推荐人流水已存在");
									...
									tleaderstaymoneydao
											.insertSelective(_tuijianTLeaderStayMoney);
								}
							}
			...
			TLeaderStayMoneyExample _TLeaderStayMoneyExample = new TLeaderStayMoneyExample();
			_TLeaderStayMoneyExample.createCriteria().andOrderIdEqualTo(order.getId())
					.andTypeEqualTo(2).andRelationMemberIdEqualTo(order.getMemberId());
			List<TLeaderStayMoney> liushuiList = tleaderstaymoneydao
					.selectByExample(_TLeaderStayMoneyExample);
			if (liushuiList != null && liushuiList.size() > 0) {
