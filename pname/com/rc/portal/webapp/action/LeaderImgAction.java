...
public class LeaderImgAction extends BaseAction{
	 ...
	 * 修改图片信息
	 */
	public void upLeaderImg() throws Exception{
		String id = getRequest().getParameter("id");
		String sort = getRequest().getParameter("sort");
		if(null!=id&&!"".equals(id)&&null!=sort&&!"".equals(sort)){
			this.getRequest().setCharacterEncoding("utf-8");
		    this.getResponse().setContentType("text/plain;charset=utf-8");
		    CImage record = new CImage();
		    record.setWeight(Integer.parseInt(sort));
		    CImageExample example = new CImageExample();
		    example.createCriteria().andIdEqualTo(Integer.parseInt(id));
		    int n = cimagemanager.updateByExampleSelective(record, example);
	...
	public String goPage() throws ParseException{
		...
		if(null!=model){
			...
			rs.setP_pageSize(10);
			pw = opensqlmanage.selectForPageByMap_drug(map, "c_image.countImages", "c_image.selectImages", rs.getP_curPage(), rs.getP_pageSize());
		}
		return "uploadPage";
	}
	/**
	...
	public void ajaxfabu() throws Exception{
		...
		if(StringUtils.hasText(ids)){
			...
			long n = cimagemanager.updateByExampleSelective(record, example);
			out.print(n);
			out.close();
		}
	}
