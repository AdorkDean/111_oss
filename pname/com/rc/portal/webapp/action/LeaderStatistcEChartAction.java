...
public class LeaderStatistcEChartAction extends BaseAction {
	
	private Condition model = new Condition();
	private PageWraper pw = new PageWraper();
	private PageResult rs = new PageResult(); 
	private OpenSqlManage opensqlmanage;
	private List<Map> baseList = new ArrayList<Map>();
	private String url;
	
	
	public String getLeaderCountBase() throws Exception{
		return "getLeaderCountBase";
	}
	
	public void getLeaderDate() throws Exception{
		...
		if(model.getStartDate()!=null&&!"".equals(model.getStartDate().trim())){
			map.put("halfMonth", model.getStartDate().trim());
		}else{
			chart=new GregorianCalendar();
			chart.add(Calendar.DAY_OF_MONTH, -15);
			map.put("halfMonth", sdf.format(chart.getTime()));
		}
		if(model.getEndDate()!=null&&!"".equals(model.getEndDate().trim())){
			map.put("yesterday", model.getEndDate().trim());
		}else{
			chart=new GregorianCalendar();
		...
		List lxxz=new ArrayList();
		List xfxz=new ArrayList();
		List yhxfxz=new ArrayList();
		if(model.getChartType()!=null&&model.getChartType()==3){
			if(model.getMobile()!=null&&!"".equals(model.getMobile().trim())){
				map.put("mobile", model.getMobile().trim());
				List<Map> dateList= opensqlmanage.selectForListByMap_drug(map, "tj_member_leader_add_count.selectDateBaseList");
				List<Map> xfList= opensqlmanage.selectForListByMap_drug(map, "tj_member_leader_add_count.selectLeaderListById");
				for(int i=0;i<dateList.size();i++){
					ydate.add(dateList.get(i).get("data_str"));
					boolean f=false;
					for(int j=0;j<xfList.size();j++){
						if(xfList.get(j).get("date_str").equals(dateList.get(i).get("data_str"))){
							f=true;
							yhxfxz.add(xfList.get(j).get("powderCount"));
						}
					}
					if(!f){
						yhxfxz.add(0);
				...
				tem.put("ydate", ydate);
				tem.put("yhxfxz", yhxfxz);
				tem.put("flag", 1);
				tem.put("mobile",model.getMobile().trim());
			}else{
				tem.put("flag", 0);
			}
		}else{
			baseList=opensqlmanage.selectForListByMap_drug(map, "tj_member_leader_add_count.selectBaseList");
			for(int i=0;i<baseList.size();i++){
				ydate.add(baseList.get(i).get("date_str"));
				lxxz.add(baseList.get(i).get("leader_count"));
				xfxz.add(baseList.get(i).get("powder_count"));
			}
			tem.put("ydate", ydate);
			tem.put("lxxz", lxxz);
			tem.put("xfxz", xfxz);
	...
	public void getDateList() throws Exception{
		...
		if(model.getStartDate3()!=null&&!"".equals(model.getStartDate3().trim())){
			map.put("halfMonth", model.getStartDate3().trim());
		}else{
			chart=new GregorianCalendar();
			chart.add(Calendar.DAY_OF_MONTH, -15);
			map.put("halfMonth", sdf.format(chart.getTime()));
		...
		if(model.getEndDate3()!=null&&!"".equals(model.getEndDate3().trim())){
			map.put("yesterday", model.getEndDate3().trim());
		}else{
			chart=new GregorianCalendar();
			chart.add(Calendar.DAY_OF_MONTH, -1);
			map.put("yesterday", sdf.format(chart.getTime()));
		}
		pw = opensqlmanage.selectForPageByMap_drug(map, "tj_member_leader_add_count.selectStatistcListCount", "tj_member_leader_add_count.selectStatistcListByPage", rs.getP_curPage(), rs.getP_pageSize());
	...
	public String exportDateList() throws Exception{
		...
		List<Map> list=opensqlmanage.selectForListByMap_drug(map, "tj_member_leader_add_count.selectDateListExcel");
		String path=getRequest().getSession().getServletContext().getRealPath("");
				path=path+"/excel_temp/"+UtilDate.getDate()+"健康领秀统计报表"+System.currentTimeMillis()+".xls";
		List lists = export();
	...
	//存放二维数组
			public String[][] getArray(List couponList) {
				String[][] s = new String[couponList.size()][7];
				for (int i = 0; i < couponList.size(); i++) {
					Map m = (Map)couponList.get(i);
					if(m.get("date_str")!=null&&!"".equals(m.get("date_str").toString())){
						s[i][0]=m.get("date_str").toString();
					}else{
						s[i][0]="";
					}
					if(m.get("leader_sum")!=null&&!"".equals(m.get("leader_sum").toString())){
						s[i][1]=m.get("leader_sum").toString();
					}else{
						s[i][1]="0";
					}
					if(m.get("powder_sum")!=null&&!"".equals(m.get("powder_sum").toString())){
						s[i][2]=m.get("powder_sum").toString();
					}else{
						s[i][2]="0";
					}
					if(m.get("member_sum")!=null&&!"".equals(m.get("member_sum").toString())){
						s[i][3]=m.get("member_sum").toString();
					}else{
						s[i][3]="0";
					}
					if(m.get("leader_count")!=null&&!"".equals(m.get("leader_count").toString())){
						s[i][4]=m.get("leader_count").toString();
					}else{
						s[i][4]="0";
					}
					if(m.get("powder_count")!=null&&!"".equals(m.get("powder_count").toString())){
						s[i][5]=m.get("powder_count").toString();
					}else{
						s[i][5]="0";
					}
					if(m.get("member_count")!=null&&!"".equals(m.get("member_count").toString())){
						s[i][6]=m.get("member_count").toString();
					}else{
						s[i][6]="0";
					}
				}
...
/*	public void getDateList() throws Exception{
	...
	public List<Map> getBaseList() {
		return baseList;
	}

	public void setBaseList(List<Map> baseList) {
