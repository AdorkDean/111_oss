...
<body> 
    ...
    <form action="${base}/member/grade!update.action" method="post" name="addInfo" id="addInfo" enctype="multipart/form-data">
    
    	<div class="m-contents">
    		<input type="hidden" name="memberGrade.id" value="${memberGrade.id}"/>
	    	<div class="labelnamese">等级名称：</div>
	    	<input type="text" name="memberGrade.name" class="i-text-i" id="" value="${memberGrade.name}"/>
    	</div>
    	
    	<div class="m-contents" style="margin-top:10px;">
	    	<div class="labelnamese">优惠比例：</div>
	    	<input type="text" name="memberGrade.enterpriseDiscount" class="i-text-i" id="" value="${memberGrade.enterpriseDiscount}"/>
    	</div>
    	
    	<div class="m-contents" style="margin-top:10px;">
	    	<div class="labelnamese">消费金额：</div>
	    	<input type="text" name="memberGrade.consumePrice" class="i-text-i" id="" value="${memberGrade.consumePrice}"/>
    	</div>
    	
    	<div class="m-contents" style="margin-top:10px;">
	    	<div class="labelnamese">默认等级：</div>
	    	<input type="checkbox" name="memberGrade.isDefault" class="i-text-cb" id="" <#if memberGrade.isDefault>checked</#if> value="true"/>
    	</div>
    	<!-- 按钮 -->
    	<div class="m-contents" style="margin-top:20px;">
	    	<div class="labelnamese"></div>
	    	<@security.authorize ifAnyGranted="ROLE_ADMIN,ROLE_HUIYUANDENGJI_BIANJI_TIJIAO">
