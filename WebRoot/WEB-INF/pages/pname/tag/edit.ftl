...
<body>
    ...
    <form method="post" id="form_id" enctype="multipart/form-data" action="${base}/label/tag!update.action">
    	<input type="hidden" name="tag.id" id="name" value="${tag.id?default('')}"/>
    	<div class="m-contents" style="margin:0px 0px 0px 10px;">
		    ...
		    <input type="text" name="tag.name" id="name" style="float:left;width:200px;height:30px;line-height:30px;border:#ccc 1px solid;" value="${tag.name?default('')}" maxlength="45"/>
    	...
    	<div class="m-contents" style="margin:10px 0px 0px 10px;">
    		...
    		<select id="type" name="tag.type" style="float:left;width:60px;height:30px;border:#ccc 1px solid;">
    			...
    			<option value="0" <#if tag.type == 0>selected="selected"</#if>>会员</option>
    		</select>
    	...
    	<div class="m-contents" style="margin-top:10px;">
    		<@security.authorize ifAnyGranted="ROLE_ADMIN,ROLE_BIAOQIAN_BIANJI_BAOCUN">
