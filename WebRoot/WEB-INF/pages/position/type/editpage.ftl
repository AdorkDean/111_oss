...
<body>
    ...
    <form action="" method="get" name="addInfo" id="addInfo">
    	...
    	<div class="m-contents" style="margin-top:5px;">
	    	...
	    	<input type="text" name="cPositionType.name" class="i-text-i" id="ctypeName" value="${cPositionType.name}"/>
    	</div>
    	
    	<!-- 按钮 -->
    	<div class="m-contents" style="margin-top:30px;">
	    	<div class="labelnamese"></div>
	    	<input type="button" class="btn01" id="submit-btn" value="提交" onclick="editPositionType(${cPositionType.id})" style="margin-left:10px;"/>
...
function editPositionType(id)
{
	...
	$.ajax(
	{
		url:"${base}/position/managePositionType!editPositionType.action?id="+id,
		type:"post",
		data:formInfo,
		success:function(data)
		{
			if(data == 1)
			{
				$alert("error","该位置类别已经存在，数据修改失败！");
			}
			if(data == 0)
			{
				$alert("success","恭喜您，数据修改成功！");
