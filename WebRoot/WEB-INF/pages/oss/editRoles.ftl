...
<form action="${base}/user/useroperate!toEditRole.action" method="post" name="form1" id="rolesub">
        ...
        <td height="24" bgcolor="#576d85"> 
			<table width="100%" border="0" cellspacing="0" cellpadding="0" >
		          <tr>
		                ...
		                <td width="94%" valign="bottom" class="l_tit">用户角色修改</td>            
		          </tr>
		        </table>        
        </td>
      </tr>
  ...
  <tr>
    <td><table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#99c3db" class="w_td" >
      ...
      <#list sysRoles?if_exists as sysrole>
			             	<#assign ckd="">	             	
			             	<#list userRoles?if_exists as userrole>
			      ...
			      <tr>			        
			        <td height="20" align="center" style="width:5%;" class="bc">
			        	<input name="showroles" type="checkbox" style="display:none" checked="checked" value="${sysrole.id?default(0)}" />
			        	<input name="rolesid" type="checkbox" ${ckd?default('')} value="${sysrole.id?default(0)}" id="${sysrole.id?default(0)}" )"/>
			        </td>
			        <td class="bc table_left" style="width:40%;">${sysrole.roleDesc?default('')}</td>
			        <td class="bc table_left" style="width:45%;">${sysrole.roleName?default('')}</td>
			      </tr>
      </#list>
...
$("#roleupdate").click(function(){
		$("#rolesub").attr("action","${base}/user/useroperate!editUserRoles.action");
		$("#rolesub").submit();
	}
);
