...
<body>
    ...
    <form action="${base}/leader/leaderAnnouncement!update.action" method="post" name="form1" id="form1" enctype="multipart/form-data">
    	<input type="hidden" name="tlxAnnouncement.id" id="id" value="${tlxAnnouncement.id?default('')}"/>
    	<div class="m-contents" style="margin-top:5px;">
	    	<div class="labelnamese">领秀通告内容：</div>
	    	<textarea class="i-text-i" style="width:250px;height:100px;" id="acCentent" name="tlxAnnouncement.centent"  maxlength="100">${tlxAnnouncement.centent?default('')}</textarea>
	    	<span style="color:red" id="sidkeyword"></span>
    	</div>
    	
    	<div class="m-contents" style="margin-top:7px;">
	    	<div class="labelnamese">权重：</div>
	    	<input type="text" class="i-text-i" name="tlxAnnouncement.weight" maxlength="3"  id="weight" value="${tlxAnnouncement.weight?default('')}" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')"/>
    	</div>
    	<div style="position:relative;" class="m-contents" style="margin-top:7px;">
	    	<div class="labelnamese">类型：</div>
	    	<input type="radio" style="width:20px; height:20px; margin-top:5px;" class="i-text-i" name="tlxAnnouncement.announcementType" value="1" <#if tlxAnnouncement.announcementType == 1>checked</#if>/><span style="position:absolute; left:198px; top:8px;">中心通告</span>
	    	<input type="radio" style="width:20px; height:20px; position:absolute; left:250px; top:5px;" class="i-text-i" name="tlxAnnouncement.announcementType" value="2" <#if tlxAnnouncement.announcementType == 2>checked</#if>/><span style="position:absolute; left:286px; top:8px;">排行通告</span>
    	</div>
    	<!-- 按钮 -->
