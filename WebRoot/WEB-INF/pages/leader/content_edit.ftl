...
<body>
    ...
    <form action="${base}/leader/leaderContent!update.action" method="post" name="form1" id="form1">
    	<div class="m-contents" style="margin-top:5px;">
	    	<div class="labelnamese">标题：</div>
	    	<input type="text" class="i-text-i" value="${obj.tile?default('')}"  id="tile" name="obj.tile" style="width:250px;"/>
	    	<input type="hidden" name="obj.id" id="id" value="${obj.id?default('')}"/>
    	</div>
    	<div class="m-contents" style="margin-top:7px;">
	    	<div class="labelnamese">内容：</div>
	    	<textarea class="i-text-i" style="width:250px;height:300px;" id="context" name="obj.context" maxlength="250">${obj.context?default('')}</textarea>
    	</div>
    	<div class="m-contents" style="margin-top:7px;">
	    	<div class="labelnamese">权重：</div>
	    	<input type="text" class="i-text-i" name="obj.weight" maxlength="3" style="width:250px;" id="weight" value="${obj.weight?default('')}" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')"/>
    	</div>
    	<!-- 按钮 -->
    	<div class="m-contents" style="margin-top:30px;">
	    	<div class="labelnamese"></div>
	    	<input type="submit" class="btn01" value="修改" style="margin-left:10px;"/>
