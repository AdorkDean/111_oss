...
<#if obj_index<6 >	
	<li>
    	<input type="hidden" value="${obj_index+1}" id="c-i-7-${obj_index+1}"/>
    	<a class="collect-list <#if obj_index == 0>collect-curr</#if>" id="sec-pro-cat-07-${obj_index+1}" href="${obj.cposition.imgLink?if_exists}" target="_blank">${obj.cposition.title?if_exists}</a>
